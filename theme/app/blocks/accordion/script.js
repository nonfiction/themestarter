...
const initAccordions = () => {
  ...
  accordions.forEach((accordion) => {
    const items = [...accordion.querySelectorAll('.wp-block-nf-accordion-item')];
    ...
    const animateClose = (item) => {
      ...
      if (!slide || item.dataset.accordionState === 'closing' || !item.classList.contains('slide-open')) {
        return;
      }
    ...
    const animateOpen = (item) => {
      const slide = item.querySelector('.accordion-slide');
      if (!slide || item.dataset.accordionState === 'opening' || item.classList.contains('slide-open')) {
        return;
      }
